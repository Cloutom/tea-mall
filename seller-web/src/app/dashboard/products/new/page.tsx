...
import { Upload, X, Plus, ChevronLeft, Loader2, Star, Package, Thermometer, Leaf, Tag, Percent, Settings2 } from 'lucide-react';
...
export default function NewProductPage() {
  ...
  const deleteCatMutation = useMutation({
    mutationFn: (id: string) => productApi.deleteStoreCategory(id),
    onSuccess: () => { refetchStoreCats(); toast.success('삭제되었습니다.'); },
  });

  const { register, handleSubmit, formState: { errors }, watch, setValue } = useForm({
    defaultValues: {
      name: '', description: '', price: '', originalPrice: '', discountStartAt: '', discountEndAt: '',
      stock: '', unit: '개', weight: '',
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { toast.success('상품이 등록되었습니다!'); router.push('/dashboard/products'); },
    onError: (e: any) => toast.error(e.response?.data?.error || '상품 등록에 실패했습니다.'),
  });

  const onSubmit = (data: any) => {
    if (!thumbnail) { toast.error('대표 이미지를 업로드해주세요.'); return; }
    ...
    if (aromaProfile.length === 0) { toast.error('향미 프로필을 최소 1개 선택해주세요.'); return; }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4 pb-10 px-4 sm:px-0 animate-fade-in">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg sm:text-xl font-bold text-gray-900">상품 등록</h1>
          <p className="text-gray-500 text-xs sm:text-sm">새로운 차 상품을 등록하세요</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <section className="card space-y-4">
          ...
          <div>
            ...
            <input {...register('name', { required: '상품명을 입력해주세요' })} placeholder="예: 유기농 제주 세작 녹차 50g" className="input-base" />
            {errors.name && <p className="text-red-500 text-xs mt-1">{errors.name.message as string}</p>}
          </div>

          <div>
            <label className="label-base">상품 설명</label>
            <textarea {...register('description')} placeholder="소비자가 첫눈에 이해할 수 있도록 상품의 핵심 특징을 요약해주세요" className="input-base h-20 resize-none" />
          </div>
                ...
                <div className="mt-2 p-3 bg-gray-50 rounded-xl border border-gray-200 space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <input value={newCatName} onChange={(e) => setNewCatName(e.target.value)}
                      placeholder="카테고리 이름 (예: 봄차 특선)" className="input-base flex-1" />
          ...
          <label className="flex items-start sm:items-center gap-3 p-3 bg-amber-50 border border-amber-200 rounded-xl cursor-pointer hover:bg-amber-100 transition-colors">
            <input type="checkbox" {...register('isSignature')} value="true" className="w-4 h-4 mt-0.5 sm:mt-0 accent-amber-500 shrink-0" />
            <div>
        ...
        <section className="card space-y-4">
          ...
          <div>
            <label className="label-base">대표 이미지 * (목록·검색에 표시)</label>
            <div onClick={() => thumbnailRef.current?.click()}
              className={clsx('border-2 border-dashed rounded-xl p-5 text-center cursor-pointer transition-all',
                thumbnailPreview ? 'border-tea-300 bg-tea-50' : 'border-gray-200 hover:border-tea-300 hover:bg-tea-50')}>
              {thumbnailPreview ? (
                ...
                <div><Upload size={28} className="mx-auto text-gray-300 mb-2" /><p className="text-sm text-gray-500">클릭하여 업로드</p><p className="text-xs text-gray-400 mt-1">JPG, PNG, WEBP 최대 10MB</p></div>
              )}
            </div>
          ...
          <div>
            <label className="label-base">추가 이미지 (최대 9개 — 상세 페이지에서 세로로 표시)</label>
            ...
            <input ref={imagesRef} type="file" accept="image/*" multiple className="hidden"
              onChange={(e) => {
                ...
                if (images.length + files.length > 9) { toast.error('최대 9개'); return; }
                setImages((p) => [...p, ...files]);
          ...
          {watchOriginal && parseFloat(watchOriginal) > 0 && (
            <div>
              <label className="label-base flex items-center gap-1.5">
                <Tag size={13} className="text-gray-400" /> 할인 기간 <span className="text-gray-400 font-normal text-xs">(선택 — 기간 외엔 정가로 노출)</span>
              </label>
        ...
        <section className="card space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input {...register('teaOrigin', { required: '원산지를 입력해주세요' })} placeholder="예: 전남 보성, 중국 운남성, 대만 아리산" className="input-base" />
            ...
            <div>
              ...
              <select {...register('harvestSeason', { required: '수확 시기를 선택해주세요' })} className="input-base">
                <option value="">선택</option>
                {HARVEST_SEASONS.map((h) => <option key={h} value={h}>{h}</option>)}
              </select>
              {errors.harvestSeason && <p className="text-red-500 text-xs mt-1">{errors.harvestSeason.message as string}</p>}
            </div>
            <div>
              <label className="label-base">가공 방법 *</label>
              <select {...register('processingMethod', { required: '가공 방법을 선택해주세요' })} className="input-base">
                <option value="">선택</option>
                {PROCESSING_METHODS.map((p) => <option key={p} value={p}>{p}</option>)}
              </select>
              {errors.processingMethod && <p className="text-red-500 text-xs mt-1">{errors.processingMethod.message as string}</p>}
            </div>
            <div>
              <label className="label-base">카페인 함량 *</label>
              <select {...register('caffeineLevel', { required: '카페인 함량을 선택해주세요' })} className="input-base">
                <option value="">선택</option>
                {CAFFEINE_LEVELS.map((l) => <option key={l.value} value={l.value}>{l.label}</option>)}
              </select>
              {errors.caffeineLevel && <p className="text-red-500 text-xs mt-1">{errors.caffeineLevel.message as string}</p>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              {errors.liquidColor && <p className="text-red-500 text-xs mt-1">수색을 선택해주세요</p>}
            </div>
            <div>
              ...
              {errors.body && <p className="text-red-500 text-xs mt-1">바디감을 선택해주세요</p>}
            </div>
          </div>
        </section>
        ...
        <section className="card space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">소비자가 비교하기 쉽도록 강도를 0~5로 설정해주세요</p>
            </div>
          ...
          <div>
            <label className="label-base">향 특성 (필수 — 복수 선택 가능)</label>
            ...
            {selectedAromas.length === 0 && (
              <p className="text-xs text-gray-400 mt-1">최소 1개 선택해주세요</p>
            )}
          </div>
        </section>
        ...
        <section className="card space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input {...register('brewingTemp', { required: '권장 물 온도를 입력해주세요' })} placeholder="예: 70~80°C, 95°C 이상" className="input-base" />
              {errors.brewingTemp && <p className="text-red-500 text-xs mt-1">{errors.brewingTemp.message as string}</p>}
            </div>
            <div>
              <label className="label-base">우리는 시간 *</label>
              <input {...register('brewingTime', { required: '우리는 시간을 입력해주세요' })} placeholder="예: 2~3분, 30초~1분" className="input-base" />
              {errors.brewingTime && <p className="text-red-500 text-xs mt-1">{errors.brewingTime.message as string}</p>}
            </div>
          </div>
          <div>
        ...
        <section className="card space-y-3">
          <h2 className="font-semibold text-gray-900 text-sm sm:text-base flex items-center gap-2">
            ...
            <span className="text-xs font-normal text-gray-400">(소비자 취향 매칭에 사용)</span>
          </h2>
          <p className="text-xs text-gray-400">이 상품의 향미 특성을 선택하세요 (필수 1개, 최대 3개). 소비자의 Tea MBTI 결과에 따라 추천됩니다.</p>
        ...
        {/* ── 태그 ── */}
        <section className="card space-y-3">
          <h2 className="font-semibold text-gray-900 text-sm sm:text-base flex items-center gap-2"><Tag size={16} className="text-gray-400" /> 태그</h2>
          <div className="flex gap-2">
            <input value={tagInput} onChange={(e) => setTagInput(e.target.value)}
              ...
              placeholder="태그 입력 후 Enter (예: 유기농, 선물용)" className="input-base flex-1" />
        ...
        <section className="card space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            {[{ value: 'true', label: '즉시 판매', desc: '등록 즉시 소비자에게 노출' }, { value: 'false', label: '비공개 (임시저장)', desc: '나중에 직접 활성화' }].map((o) => (
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mb-2">등록 후 선택한 기간 동안 상품에 "신상품" 뱃지가 표시됩니다</p>
            <div className="flex flex-wrap gap-2">
                  ...
                  <div key={vi} className="flex items-center gap-2">
                    ...
                    <label className="flex items-center gap-1 text-xs text-gray-500 shrink-0">
                      <input type="checkbox" checked={val.soldOut}
                        onChange={e => updateOptionValue(gi, vi, 'soldOut', e.target.checked)}
                        className="rounded" />
                      품절
                    </label>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <Link href="/dashboard/products" className="btn-secondary flex-1 py-3 text-center">취소</Link>
          <button type="submit" disabled={createMutation.isPending} className="btn-primary flex-1 py-3">
            {createMutation.isPending ? <Loader2 size={17} className="animate-spin" /> : null}
